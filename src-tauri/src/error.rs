...
impl From<AppError> for PublicError {
    fn from(error: AppError) -> Self {
        let (code, message, guidance, retryable) = match error {
            AppError::CaptureAlreadyActive => (
                ErrorCode::CaptureAlreadyActive,
                "A capture is already active.",
                "Finish or cancel the active selector before trying again.",
                true,
            ),
            AppError::CaptureCancelled => (
                ErrorCode::CaptureCancelled,
                "Capture was cancelled.",
                "Start another capture when ready. The clipboard was unchanged.",
                true,
            ),
            ...
            AppError::CaptureImageInvalid => (
                ErrorCode::CaptureImageInvalid,
                "The selected image could not be decoded safely.",
                "Select a different region and try again.",
                true,
            ),
            AppError::CaptureTooLarge => (
                ErrorCode::CaptureTooLarge,
                "The selected region is too large.",
                "Select a smaller region.",
                true,
            ),
            AppError::TemporaryCleanupFailed => (
                ErrorCode::TemporaryCleanupFailed,
                "A temporary capture could not be removed.",
                "Quit the application and inspect its runtime directory before continuing.",
                false,
            ),
            AppError::OcrEngineUnavailable => (
                ErrorCode::OcrEngineUnavailable,
                "Tesseract is not available.",
                "Install tesseract-ocr and tesseract-ocr-eng.",
                true,
            ),
            AppError::OcrLanguageMissing => (
                ErrorCode::OcrLanguageMissing,
                "English OCR data is missing.",
                "Install the tesseract-ocr-eng package.",
                true,
            ),
            AppError::OcrTimedOut => (
                ErrorCode::OcrTimedOut,
                "OCR took too long and was stopped.",
                "Select a smaller region and try again. The clipboard was unchanged.",
                true,
            ),
            AppError::OcrFailed => (
                ErrorCode::OcrFailed,
                "Tesseract could not recognize the selected region.",
                "Try a clearer or smaller region. The clipboard was unchanged.",
                true,
            ),
            AppError::OcrEmptyResult => (
            ...
            AppError::ClipboardWriteFailed => (
                ...
                "Keep the preview open and retry the copy action.",
                true,
            ),
            AppError::SettingsInvalid => (
                ErrorCode::SettingsInvalid,
                "The settings are invalid.",
                "Review the selected values or reset settings.",
                true,
            ),
            AppError::SettingsWriteFailed => (
                ErrorCode::SettingsWriteFailed,
                "Settings could not be saved.",
                "Check configuration-directory permissions and try again.",
                true,
            ),
            AppError::UnsupportedEnvironment => (
                ErrorCode::UnsupportedEnvironment,
                "This desktop environment is not supported.",
                "Use Ubuntu GNOME on Wayland or X11.",
                false,
            ),
            AppError::Internal => (
                ErrorCode::InternalError,
                "An internal operation failed.",
                "Restart the application and inspect safe diagnostics.",
                true,
            ),
        };
        Self {
