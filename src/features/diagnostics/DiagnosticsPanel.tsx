    ...
    <div className="space-y-5">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
          This report excludes screenshots, recognized text, clipboard contents, and temporary
          paths.
        </p>
      </header>
      <dl className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
        {rows.map(([key, value]) => (
          <div
            key={key}
            className="grid gap-1 border-b border-slate-200 py-3 last:border-b-0 dark:border-slate-800 sm:grid-cols-[14rem_1fr]"
          >
